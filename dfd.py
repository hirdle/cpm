...
def line_align_forward(v, thereshold=30): # выравнивание "вперёд"
    while LS_left.reflection() > thereshold or LS_right.reflection() > thereshold:
        left_motor.run(v)
        right_motor.run(v)
        if LS_left.reflection() < thereshold:
            while LS_left.reflection() < thereshold:
                left_motor.run(-v)
                right_motor.run(-v/2)
            wait(100)
        if LS_right.reflection() < thereshold:
            while LS_right.reflection() < thereshold:
                left_motor.run(-v/2)
                right_motor.run(-v)
            wait(100)
        wait(10)
...
def per_count(v,total_len,thereshold=30, round_=25, add=15): # считывание чб штрихкода по длинам
        pers_list_black=[]
        pers_list_white=[]
        # line_align_forward(50, thereshold)
        start_deg = left_motor.angle()
        while left_motor.angle()-start_deg < mm_to_deg(total_len):
            old_deg = left_motor.angle()
            while LS_left.reflection() < thereshold:
                left_motor.run(v)
                right_motor.run(v)
            pers_list_black.append(floor(deg_to_mm(left_motor.angle()-old_deg+add)/round_)*round_)
            old_deg = left_motor.angle()
            if left_motor.angle()-start_deg < mm_to_deg(total_len):
                while LS_left.reflection() >= thereshold and left_motor.angle()-start_deg < mm_to_deg(total_len):
                    left_motor.run(v)
                    right_motor.run(v)
                pers_list_white.append(floor(deg_to_mm(left_motor.angle()-old_deg+add)/round_)*round_)
        motors_stop()
...
def code_read():
    ...
    ride_mm_no_stop(200, deg_target)
    for i in range(7):
        # ev3.speaker.beep(100)
